...
module.exports = {
  ...
  async execute(message: Message, args: Array<string>) {
    ...
    const fulllang: Langtypes = JSON.parse(
    ...
    if (!args[0] || args[0] === "say") {
      ...
      quotes
        .where(admin.firestore.FieldPath.documentId(), ">=", key2)
        .limit(1)
        .get()
        .then(
          (snapshot: {
            ...
            if (snapshot.size > 0) {
              snapshot.forEach((doc: { id: any; data: () => any }) => {
                sendRandomFact(
                  doc.id,
                  message,
                  qdb.get(`guild.${message.guild?.id}`).lang,
                  args
                );
              });
            } else {
...
async function sendRandomFact(
  ...
  if (!doc.data()!.owner) {
    let upmbed = new Discord.MessageEmbed()
      ...
      .setFooter(
        `${message.author.tag} • This is a template fact • Reggeltbot facts`,
    ...
    let upmbed = new Discord.MessageEmbed()
      ...
      .addField(lang.add, lang.addF)
      .setFooter(
        `${message.author.tag} • Reggeltbot facts`,
        message.author.avatarURL({ dynamic: true }) || undefined
      )
      .setTimestamp(Date.now());
...
async function getRandomFactWithId(
  id: any,
  message: Message,
  args: Array<string>
) {
