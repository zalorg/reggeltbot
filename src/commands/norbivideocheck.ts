...
async function sendissue(bot: Discord.Client, message: Discord.Message, args: Array<string>, channel: string) {
    console.log(args)

    if(message.channel.isText()) {
        const msgch: any = message.channel;
        const collector = new Discord.MessageCollector(msgch, m => m.author.id === message.author.id, { time: 60000 });
        let array: any[] = [];
        message.reply('Video: ')
        ...
        collector.on('collect', msg => {
            if(!array[0]) {
                ...
                db.collection('waik').doc('norbi').collection('videocheck').add({
                    video: array[0],
                    time: array[1],
                    issue: array[2],
                    status: 'new',
