  ...
  async execute(message: Discord.Message, args: string[], bot: Discord.Client) {
    ...
    if (channel.isText()) {
      const embed = new Discord.MessageEmbed()
        ...
        .setFooter(
          `${message.author.tag} • Reggeltbot poll (Beta)`,
      ...
      message.channel.send(embed).then((m) => {
        ...
        collector.on("collect", async (msg: Discord.Message) => {
          let reactions: Discord.Emoji[] =
            qdb.get(`temp.guildcounter.${message.guild?.id}.reactions`) || [];

          if (msg.content === "stop" || msg.content === "cancel")
            return msg.channel
              .send("Canceled")
              .then((m) =>
          ...
          switch (qdb.get(`temp.guildcounter.${message.guild?.id}.state`)) {
                ...
                if (reactions.length === 0) {
                  ...
                  let embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(
                      `
                                    ...
                                    \nCurrent channel: <#${
                                      qdb.get(
                                        `temp.guildcounter.${message.guild?.id}.channel`
                                      ).id
                                    }>
                                    `
                    )
                  ...
                  m.edit(embed).then((m2) => {
                    if (msg.deletable) {
                      ...
                      qdb.set(
                        `temp.guildcounter.${message.guild?.id}.state`,
                        2
                      );
                    }
                  });
            ...
            case 2:
              if (msg.content === "stop") {
                ...
                qdb.set(
                  `temp.guildcounter.${message.guild?.id}.content`,
                  msg.content
                );
                ...
                m.edit(embed).then((m2) => {
                  ...
                  const coll = m2.createReactionCollector(
                    (react, user) => user.id === msg.author.id
                  );

                  coll.on("collect", (r, u) => {
                    if (r.emoji.id === "809931766642245663") {
                      const chnlid = qdb.get(
                        `temp.guildcounter.${message.guild?.id}.channel`
                      ).id;
            ...
            default:
              console.log(
                qdb.get(`temp.guildcounter.${message.guild?.id}.state`)
              );
                ...
                await chnlr?.fetch().then((chnl) => {
                  if (!chnl) {
                    ...
                    let embed2 = new Discord.MessageEmbed()
                      .setTitle("Reggeltbot poll create")
                      .setDescription(
                        `Okay, so i will start the poll in ${chnl}, now please add the option, by reacting to this message 
                    ...
                    m.edit(embed2).then((m2) => {
                      ...
                      const collector2 = m2.createReactionCollector(
                        (r, u) => u
                      );
                      ...
                      collector2.on("collect", (react, user) => {
                        ...
                        qdb.set(
                          `temp.guildcounter.${message.guild?.id}.reactions`,
                          reactions
                        );
