...
module.exports = {
  ...
  async execute(
    ...
    const coinEmote = bot.emojis.cache.find(
      (e) => e.id === qdb.get("config.coinEmote")
    );
        ...
        if (args[1] === "reggeltemote") {
          ...
          if (array.find((e) => e === emote)) {
            userref
              ...
              .catch((e) => {
                message.reply(
                  "Error! this dumass probably messed something up <@423925286350880779>"
                );
              });
          } else {
      ...
      case "buy":
        ...
        if (args[1] === "emote") {
          ...
          if (!args[2]) {
            ...
            message.reply(
              `You reatched the limit (3 emotes) \n You use these emote: [${array.join(
                " "
              )}]`
            );
          } else {
            ...
            if (!userdoc.data()?.coins) {
              if (!userdoc.data()?.reggeltcount) {
                message.reply(
                  `You dont have any ${coinName}! Start whising mornigng to people in <#${
            ...
            function addemotes() {
              ...
              if (userdoc.data()?.coins >= Number(emoteBuy)) {
                ...
                message.reply(
                  `You dont have enough ${coinName} to by an emote \n Your current ${coinName}s: **${
                    userdoc.data()?.coins
                  }** ${coinEmote}`
                );
              }
      ...
      case "sell":
        ...
        if (!args[1]) {
          ...
          let embed = new Discord.MessageEmbed()
            .setTitle(
              `You must change you current reggeltemote to sell this emote!`
            )
            .addField(
            ...
            .setFooter(
              `${message.author.tag} • Reggeltbot economy (Beta)`,
...
async function help(
  message: Discord.Message,
  coinEmote: Discord.Emoji | undefined
) {
  const emoteBuy = `${qdb.get("config.emoteBuy")}`;
  const emoteSell = `${qdb.get("config.emoteSell")}`;
  const coinName = `${qdb.get("config.coinName")}`;
  ...
  let embed = new Discord.MessageEmbed()
    ...
    .addField(
      `${prefix}shop buy`,
      `   **emote** *EMOJI* : You can buy emotes for **${emoteBuy} ${coinName}${coinEmote}** and use them to customise you experimance`
    )
    .addField("\u200B", "\u200B")
    .addField(
      `${prefix}shop buy`,
      `   **emote** *EMOJI* : You can sell unwanted emotes **${emoteSell} ${coinName}${coinEmote}** and free up space in your inventory`
    )
    .addField("\u200B", "\u200B")
    .addField(
      `${prefix}shop set`,
      `**reggeltemote** *EMOJI* : You can set the reaction of you reggelt message in ${
        message.guild?.channels.cache.find((e) => e.name === "reggelt") ||
        "a new #reggelt channel made by the admin!"
