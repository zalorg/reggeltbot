...
module.exports = {
  ...
  async execute(message: Message, bot: Client) {
    ...
    const lang = JSON.parse(
      fs.readFileSync(`./lang/${currentLang}.json`, "utf8")
    ).commands.count;

    const reggeltconfig: Reggeltconfig = JSON.parse(
      fs.readFileSync(`./lang/${currentLang}.json`, "utf8")
    ).events.reggelt;
    ...
    if (!doc.exists) {
      ...
      let upmbed = new MessageEmbed()
        ...
        .addField(
          `${lang.f1}`
            .replace("%!KEYWORD%!", reggeltconfig.keyWord)
            .replace("%!COUNT%!", `${doc.data()!.reggeltcount1 || 0}`)
        ...
        .setFooter(
          `${message.author.tag} • Reggeltbot count`,
