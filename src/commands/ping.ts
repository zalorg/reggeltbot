...
module.exports = {
  ...
  async execute(bot: Client, args: any, message: Message) {
    ...
    const guildsettings: Guildconfig =
      qdb.get(`guilds.${message.guild?.id}`) || "en-US";

    const langfull: Langtypes = JSON.parse(
      fs.readFileSync(`./lang/${guildsettings}.json`, "utf8")
    );
    ...
    await axios.default
      .post(`https://discord.com/api/v8/channels/${message.channel.id}/typing`)
      .catch((err) => {
      ...
      pingss.forEach((ping) => {
        if (ping.status === 200) {
          if (ping.ping > pingval) {
            embed.addField(
              ping.name,
              `${yellowEmote} Status: **${ping.data}** Ping: **${ping.ping}ms**`.replace(
                "Status",
                `${lang.status}`
              )
            );
            ...
            embed.addField(
              ping.name,
              `${greenEmote} Status: **${ping.data}** Ping: **${ping.ping}ms**`.replace(
                "Status",
                `${lang.status}`
              )
            );
          ...
          embed.addField(
            ping.name,
            `${redEmote} Error: **${ping.err}**`.replace(
              "Error:",
              `${lang.error}:`
            )
          );
...
async function pings(bot: any, lang: any) {
  ...
  await axios.default
    .get(`${process.env.APIURL || configdoc.APIURL}/ping`)
    .then((res) => {
      array.push({
        name: lang.internal,
        status: res.status,
  ...
  await axios.default
    .get(`${process.env.RAPIURL || configdoc.RAPIURL}/ping`)
    .then((res) => {
      array.push({
  ...
  await axios.default
    .get(`https://zal1000.ew.r.appspot.com/ping`)
    .then((res) => {
      array.push({
        name: "App Engine",
        status: res.status,
