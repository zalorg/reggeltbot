...
module.exports = {
  ...
  async execute(bot: Client) {
    bot.on("guildMemberAdd", async (member) => {
      ...
      if (
        member.guild.id === "541446521313296385" ||
        member.guild.id === "738169002085449748"
      ) {
        ...
        if (a === "LIKELY" || a === "POSSIBLE" || a === "UNKNOWN") {
          ...
          if (logchannel?.isText()) {
            const embed = new MessageEmbed()
              ...
              .addField(
                "Adult",
                `${detections!.adult} (conf: ${detections?.adultConfidence})`
              )
              .addField(
                "Medical",
                `${detections!.medical} (conf: ${
                  detections?.medicalConfidence
                })`
              )
              .addField(
                "Spoof",
                `${detections!.spoof} (conf: ${detections?.spoofConfidence})`
              )
              .addField(
                "Violance",
                `${detections?.violence} (conf: ${detections?.violenceConfidence})`
              )
              .addField(
                "Racy",
                `${detections?.racy} (conf: ${detections?.racyConfidence})`
              )
