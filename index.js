...
const drep = new DRepClient('D-REP.XS5NZDT6BSVSVRO7Y7D9N8SKLUBTDLYZIR5U7.FS2QUWYA0QM3H.VEW6U8');
const dbl = new DBL('eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6Ijc0OTAzNzI4NTYyMTYyODk1MCIsImJvdCI6dHJ1ZSwiaWF0IjoxNjA0MjU4NDIyfQ.IC_QfFnA0YMB9-CRRNAL7bMPJs407iai2ni2rWii1os', bot);
...
/*
    if (message.channel.id === "763064684072796200"){
        if (message.content === `RESTART REGGELTBOT`){
            console.log("Restarting... (requested by.: Zal Bot)")
...
reggeltupdatefs()
        ...
        message.react('☕');     
            ...
            async function reggeltupdatefs3() {
              var db = admin.firestore();
            const reggeltRef = db.collection('dcusers').doc(message.author.id);
            const doc = await reggeltRef.get();
            if (!doc.exists) {
            ...
            reggeltupdatefs3()
/*
            async function allreggeltset() {
              const citiesRef = db.collection('dcusers');
              const snapshot = await citiesRef.where('reggeltcount', '!=', null).get();
              if (snapshot.empty) {
                console.log('No matching documents.');
                return;
              }  
              ...
              snapshot.forEach(doc => {
                ...
                async function reggeltupdatefs2() {
                  var db = admin.firestore();
                const reggeltRef = db.collection('dcusers').doc("all");
                const doc2 = await reggeltRef.get();
                if (!doc2.exists) {
                  reggeltRef.set({
                    reggeltcount: -1,
                    pp: message.author.avatarURL,
                  ...
                  reggeltRef.update({
                    reggeltcount: admin.firestore.FieldValue.increment(doc.data()),
                    pp: message.author.avatarURL,
                    tag: message.author.tag,
                    username: message.author.username
                  });
