...
module.exports = {
  ...
  async execute(
    ...
    if (message.guild && message.guild.id === "541446521313296385") {
      ...
      message.guild &&
      message.guild.id === "738169002085449748" &&
  ...
  yt.videos
    ...
    .then(async (r) => {
      if (channel?.isText()) {
        ...
        const msg = `${doc.data()!.msg}`
          .replace("%!AUTHOR%!", r.data.items![0].snippet?.channelTitle!)
          .replace("%!URL%!", args[0])
          .replace("%!CREATEDAT%!", r.data.items![0].snippet?.publishedAt!)
          .replace("%!DESCRIPTION%!", args[0])
          .replace("%!URL%!", args[0])
          .replace("%!TAG%!", "<@&820063378999476265>");
